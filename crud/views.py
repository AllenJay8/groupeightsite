    ...
    try:
        if request.method == 'POST':
           ...
           # if password != confirmPassword:
              #  messages.error(request, 'Password and Confirm Password do not match!') # Error message

            Users.objects.create(
                full_name=fullName,
                gender=Genders.objects.get(pk=gender), # SELECT * FROM tbl_genders WHERE id = genderId;
                birth_date=birthDate,
                address=address,
                contact_number=contactNumber,
                email=email,
                username=username,
                password=make_password(password) # Hash before save.
            ).save() # INSERT INTO tbl_users (fullName, gender, birth_date, address, contact_number, email, username, password)
                     #VALUES (fullName, gender, birthDate, address, contactNumber, email, username, password);
